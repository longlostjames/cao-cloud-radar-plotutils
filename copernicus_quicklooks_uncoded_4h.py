...
h2=axs[2].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['LDR_C'][:,:].transpose(),vmin=-35,vmax=5,cmap=cmap_hoganjet,shading='auto');
cb2=plt.colorbar(h2,ax=axs[2],orientation='vertical');
cb2.ax.set_ylabel("LDR_C (dB)");
axs[2].grid(True);
...
for file in rawfiles35[istartfile+1:]:
    ...
    if nray>0:
        ...
        axs[2].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['LDR_C'][:,:].transpose(),vmin=-35,vmax=5,cmap=cmap_hoganjet,shading='auto');
...
figfile = "radar-copernicus_uncoded_{}_last4hr.png".format(datestr);
