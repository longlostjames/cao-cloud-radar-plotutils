...
dt1 = data_date;
#dt0 = data_date - timedelta(hours=24);
dt0 = dt1 + timedelta(seconds=0);
dt0 = dt1.replace(hour=0, second=0, microsecond=0)
...
raw_path94 = os.path.join('/radar/radar-galileo/raw',datestr);

os.chdir(raw_path94);
rawfiles94 = [os.path.join(raw_path94,f) for f in glob.glob('*{}*raw.nc'.format(datestr))]
...
DS1 = nc4.Dataset(rawfiles94[iendfile], 'r',format='NETCDF4_CLASSIC');
...
#drng = 0.0; #329.7717038;

ZED_HCnew = ZED_HCnew+20.*np.log10(rng[None,:])-20.*np.log10(rng[None,:]+drng);
...
h0=axs[0].pcolormesh(dtime0[:],gate_edges,ZED_HCnew.transpose(),vmin=-40,vmax=40,cmap=cmap_hoganjet,shading='auto');
...
h1=axs[1].pcolormesh(dtime0[:],gate_edges,DS0['VEL_HC'][:,:].transpose(),vmin=-5,vmax=5,cmap=cmap_hoganjet,shading='auto');
...
try:
    ...
    h2=axs[2].pcolormesh(dtime0[:],gate_edges,DS0['LDR_HC'][:,:].transpose(),vmin=-35,vmax=5,cmap=cmap_hoganjet,shading='auto');
...
try:
    ...
    h3=axs[3].pcolormesh(dtime0[:],gate_edges,DS0['SPW_HC'][:,:].transpose(),norm=colors.LogNorm(vmin=spw_plotmin,vmax=spw_plotmax),cmap=cmap_hoganjet,shading='auto');
...
try:
    ...
    ZED_XHCnew = ZED_XHCnew+20.*np.log10(rng[None,:])-20.*np.log10(rng[None,:]+drng);
except:
    print("No ZED_XHC");
    ZED_XHCnew = np.empty([nray,ngate]);
    
axs[4].xaxis.set_major_formatter(myFmt);
h4=axs[4].pcolormesh(dtime0[:],gate_edges,ZED_XHCnew[:,:].transpose(),vmin=-40,vmax=40,cmap=cmap_hoganjet,shading='auto');
...
for file in rawfiles94[istartfile+1:]:
    ...
    if nray>0:
        ...
        rng = DS0['range'][:];
        drng = range_offset-DS0['range'].range_offset;
        ZED_HCnew = DS0['ZED_HC'][:,:]+20.*np.log10(rng[None,:])-20.*np.log10(rng[None,:]+drng);
        range_km = (rng+drng)/1000.;

        gate_edges = range_km - 29.9792458/1000.;
        gate_edges = np.append(gate_edges,gate_edges[-1]+2*29.9792458/1000.);
        try:
            axs[1].pcolormesh(dtime0[:],gate_edges,DS0['VEL_HC'][:,:].transpose(),vmin=-5,vmax=5,cmap=cmap_hoganjet,shading='auto');
            axs[2].pcolormesh(dtime0[:],gate_edges,DS0['LDR_HC'][:,:].transpose(),vmin=-35,vmax=5,cmap=cmap_hoganjet,shading='auto');
            axs[0].pcolormesh(dtime0[:],gate_edges,ZED_HCnew.transpose(),vmin=-40,vmax=40,cmap=cmap_hoganjet,shading='auto');
            axs[3].pcolormesh(dtime0[:],gate_edges,DS0['SPW_HC'][:,:].transpose(),norm=colors.LogNorm(vmin=spw_plotmin,vmax=spw_plotmax),cmap=cmap_hoganjet,shading='auto');

            ZED_XHCnew = DS0['ZED_XHC'][:,:];
            ZED_XHCnew = ZED_XHCnew+20.*np.log10(rng[None,:])-20.*np.log10(rng[None,:]-drng);
            axs[4].pcolormesh(dtime0[:],gate_edges,ZED_XHCnew[:,:].transpose(),vmin=-40,vmax=40,cmap=cmap_hoganjet,shading='auto');
        except:
...
figfile = "radar-galileo_{}.png".format(datestr);
