...
dt1 = data_date;
dt0 = data_date - timedelta(hours=4);
...
ZED_HCnew = ZED_HCnew+20.*np.log10(rng)-20.*np.log10(rng-220.0);
rng = rng - 220.0
...
h3=axs[3].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['SPW_HC'][:,:].transpose(),norm=colors.LogNorm(vmin=spw_plotmin,vmax=spw_plotmax),cmap=cmap_hoganjet,shading='auto');
