...
drng = 220;
rng = rng - drng;
ZED_HCnew = ZED_HCnew+20.*np.log10(rng)-20.*np.log10(rng-drng);

spw_plotmin = np.sqrt(1e-3);
spw_plotmax = np.sqrt(10.)

axs[0].xaxis.set_major_formatter(myFmt);
h0=axs[0].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,ZED_HCnew.transpose(),vmin=-40,vmax=40,cmap=cmap_hoganjet,shading='auto');
...
h1=axs[1].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['VEL_HC'][:,:].transpose(),vmin=-5,vmax=5,cmap=cmap_hoganjet,shading='auto');
...
h2=axs[2].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['LDR_HC'][:,:].transpose(),vmin=-35,vmax=5,cmap=cmap_hoganjet,shading='auto');
...
try:
    ...
    h3=axs[3].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['ZED_XHC'][:,:].transpose(),vmin=-40,vmax=40,cmap=cmap_hoganjet,shading='auto');
    cb3=plt.colorbar(h3,ax=axs[3],orientation='vertical');
    cb3.ax.set_ylabel("ZED_XHC (dBZ)");
    axs[3].grid(True);
...
h4=axs[4].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['SPW_HC'][:,:].transpose(),norm=colors.LogNorm(vmin=spw_plotmin,vmax=spw_plotmax),cmap=cmap_hoganjet,shading='auto');
cb4=plt.colorbar(h4,ax=axs[4],orientation='vertical');
cb4.ax.set_ylabel("SPW_HC (m$s^{-1}$)");
axs[4].grid(True);
...
for file in rawfiles94[istartfile+1:]:
    ...
    if nray>0:
        ...
        rng = np.tile(DS0['range'][:],(nray,1));
        ZED_HCnew = DS0['ZED_HC'][:,:]+20.*np.log10(rng)-20.*np.log10(rng-220.0);
        rng = rng - 220.0

        axs[1].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['VEL_HC'][:,:].transpose(),vmin=-5,vmax=5,cmap=cmap_hoganjet,shading='auto');
        axs[2].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['LDR_HC'][:,:].transpose(),vmin=-35,vmax=5,cmap=cmap_hoganjet,shading='auto');
        axs[0].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,ZED_HCnew.transpose(),vmin=-40,vmax=40,cmap=cmap_hoganjet,shading='auto');
        axs[3].pcolormesh(dtime0[:],(DS0['range'][:]-drng)/1000.,DS0['SPW_HC'][:,:].transpose(),norm=colors.LogNorm(vmin=spw_plotmin,vmax=spw_plotmax),cmap=cmap_hoganjet,shading='auto');
